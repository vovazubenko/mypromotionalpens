        ...
        public override RouteData GetRouteData(HttpContextBase httpContext)
        {
            ...
            if (data != null && DataSettingsHelper.DatabaseIsInstalled())
            {
                ...
                if (urlRecord == null)
                {
                    ...
                    string lastPart = slug.Split('_').Last();
                    if (!string.IsNullOrEmpty(lastPart))
                    {
                        urlRecord = urlRecordService.GetBySlugCached(lastPart);
                        if (urlRecord == null)
                        {
                        ...
                        var workContext1 = EngineContext.Current.Resolve<IWorkContext>();
                        var slugForCurrentLanguage1 = SeoExtensions.GetSeName(urlRecord.EntityId, urlRecord.EntityName, workContext1.WorkingLanguage.Id);
                    //    if (!String.IsNullOrEmpty(slugForCurrentLanguage1) &&
                    //!slugForCurrentLanguage1.Equals(slug, StringComparison.InvariantCultureIgnoreCase))
                    //    {

                    //        var webHelper = EngineContext.Current.Resolve<IWebHelper>();
                    //        var response = httpContext.Response;
                    //        response.Status = "302 Moved Temporarily";
                    //        response.RedirectLocation = string.Format("{0}{1}", webHelper.GetStoreLocation(), slugForCurrentLanguage1);
                    //        response.End();
                    //        return null;
                    //    }
                            ...
                            case "product":
                                {
                                    ...
                                    data.Values["FilterUrl"] = slug;
                                    data.Values["paramUrl"] = httpContext.Request.QueryString.ToString();
                                }
                                break;
                            case "category":
                                {
