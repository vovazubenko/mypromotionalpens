    ...
    public partial class SettingController : BaseAdminController
    {
        ...
        public virtual ActionResult Shipping()
        {
            ...
            if (storeScope > 0)
            {
                ...
                model.isusafreeshipping = _settingService.SettingExists(shippingSettings, x => x.isusafreeshipping, storeScope);
                model.iscontigousfreeshipping = _settingService.SettingExists(shippingSettings, x => x.iscontigousfreeshipping, storeScope);
            }
            //shipping origin
        ...
        public virtual ActionResult Shipping(ShippingSettingsModel model)
        {
            ...
            _settingService.SaveSettingOverridablePerStore(shippingSettings, x => x.isusafreeshipping, model.isusafreeshipping, storeScope, false);
            _settingService.SaveSettingOverridablePerStore(shippingSettings, x => x.iscontigousfreeshipping, model.iscontigousfreeshipping, storeScope, false);
        ...
        public virtual ActionResult GeneralCommon()
        {
            ...
            model.StoreInformationSettings.FilterSiteMapSpecificationAttributeOption = commonSettings.FilterSiteMapSpecificationAttributeOption;
            model.StoreInformationSettings.FilterSiteMapSpecificationAttributeOptionValue = commonSettings.FilterSiteMapSpecificationAttributeOptionValue;
        ...
        public virtual ActionResult GeneralCommon(GeneralCommonSettingsModel model)
        {
            ...
            commonSettings.FilterSiteMapSpecificationAttributeOption = model.StoreInformationSettings.FilterSiteMapSpecificationAttributeOption;
            commonSettings.FilterSiteMapSpecificationAttributeOptionValue = model.StoreInformationSettings.FilterSiteMapSpecificationAttributeOptionValue;
        ...
        [HttpPost]
        public virtual ActionResult DomainList(DataSourceRequest command)
        {
            ...
            var customers = _restrictionDomainService.GetRestrictionDomain(
        ...
        [NonAction]
        protected virtual DomainRestrictionModel PrepareCustomerModelForList(RestrictionDomain customer)
        {
            return new DomainRestrictionModel
            {
        ...
        ////public virtual ActionResult RestrictDomainList()
        //{
        //    if (!_permissionService.Authorize(StandardPermissionProvider.ManageSettings))
        //        return AccessDeniedView();
        ...
        //    //merge settings
        //    var model = new DomainRestrictionListModel();
        //    model.lstDomain = new List<DomainRestrictionModel>();
        //    List<RestrictionDomain> lstRestrictionDomain = _restrictionDomainService.GetRestrictionDomain();
        //    if (lstRestrictionDomain != null)
        //    {
        //        foreach (var item in lstRestrictionDomain)
        //        {
        //            model.lstDomain.Add(new DomainRestrictionModel
        //            {
        //                dId = item.Id,
        ...
        public virtual ActionResult RestrictDomainDelete(int dId)
        {
            int Id = dId;
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageSettings))
                return AccessDeniedView();
            ...
            if (Id > 0)
            {
                RestrictionDomain obj = _restrictionDomainService.GetRestrictionDomainById(Id);
                if (obj != null)
                {
                    _restrictionDomainService.DeleteDomain(obj);
                }
                else
                    throw new ArgumentException("No domain found with the specified id");
        ...
        public virtual ActionResult RestrictDomain(int Id = 0)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageSettings))
                return AccessDeniedView();
            ...
            //merge settings
            var model = new DomainRestrictionModel();
            model.dId = 0;
            model.DefaultDomainURL = _settingService.GetSettingByKey<string>("DefaulDomain","");
            if (Id > 0)
            {
                RestrictionDomain obj = _restrictionDomainService.GetRestrictionDomainById(Id);
                if (obj != null)
                {
                    model.dId = obj.Id;
                    model.DomainURL = obj.DomainURL;
                }
            }
            return View(model);
        }
        [HttpPost]
        public virtual ActionResult RestrictDomain(DomainRestrictionModel model)
        {

            RestrictionDomain obj = null;
            ...
            {
                if (model.dId > 0)
                    obj = _restrictionDomainService.GetRestrictionDomainById(model.dId);
                else
                    obj = new RestrictionDomain();

                obj.DomainURL = model.DomainURL;
                if (_restrictionDomainService.IsExists(obj.DomainURL, obj.Id) == true)
                {
                    ModelState.AddModelError("DomainExists", "Domain already exists.");
                    return View(model);
                }
                //if(Uri.CheckHostName(model.DomainURL) != UriHostNameType.Unknown)
                //{
                //    ModelState.AddModelError("DomainExists", "Domain URL is not valid.");
                //    return View(model);

                //}
                _restrictionDomainService.InsertDomain(obj);
            ...
            return RedirectToAction("RestrictDomain");
        }
         [HttpPost]
        public virtual ActionResult DefaultDomain(DomainRestrictionModel model)
        {
            ...
            {
                if (string.IsNullOrEmpty(model.DefaultDomainURL) == false)
                {
                    _settingService.SetSetting("DefaulDomain", model.DefaultDomainURL);
                }

                else
                {
                    ModelState.AddModelError("DomainURI", "Please provide default domain.");
                    
                    return View("RestrictDomain", model);
                }
