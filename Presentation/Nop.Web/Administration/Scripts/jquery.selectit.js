...
(function ($) {
    ...
    function addValue(tag, options) {
        var hiddeninput = $('<input/>').attr('type', 'hidden').val(tag.Name);
        if (options.fieldname) {
            hiddeninput.attr('name', options.fieldname);
        }
        this.before(
                    $('<span/>').addClass('selectit-option').html(tag.Name).append(hiddeninput).append(
                        $('<a/>').addClass('selectit-close').attr('title', 'remove').html('x').click(function () {
                            var supplier = {
                                SupplierId: $(this).parent().attr("SupplierId"),
                                Name: $(this).parent().find("input").val(),
                            }
                            if (Window.HandelTagRemove) {
                                Window.HandelTagRemove(supplier);
                            }
                            $(this).parent().remove();
                        })
                    ).attr("SupplierId", tag.SupplierId)
                );
    }
            ...
            if (method === 'values') {
                ...
                if (value.length) {
                    for (var vi = 0; vi < value.length; vi++) {
                        addValue.call($(this).find('.selectit-new'), value[vi].Name, $(this).data('selectit-options'));
                    }
                }
                else if (value && value.Name) {
                    addValue.call($(this).find('.selectit-new'), value, $(this).data('selectit-options'));
                }
            }
