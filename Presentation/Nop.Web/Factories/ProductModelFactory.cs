...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual ProductOverviewModel.ProductPriceModel PrepareProductOverviewPriceModel(Product product, bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            priceModel.Setup = String.Format(_priceFormatter.FormatPrice(product.SetupFee));
            priceModel.SetupValue = product.SetupFee;
            return priceModel;
        }
        ...
        protected virtual ProductDetailsModel.ProductPriceModel PrepareProductPriceModel(Product product)
        {
            ...
            model.SetupFee = product.SetupFee;
                                        ...
                                        if (attributeModel.ProductAttributeId == 12 || attributeModel.Name.Contains("comment"))
                                        {
                                            ...
                                            foreach (var item in attributeModel.Values)
                                            {
                                                if (cnt < enteredText.Count())
                                                {
                                                    ...
                                                    attributeModel.Values[cnt].DefaultValue = enteredText[cnt];
                                                    cnt++;
                                                }
                                            }
                                        }
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                var DiscountRanges = (from a in applied
                                        select new ProductDetailsModel.DiscountRange
                                        {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                IsEQP=product.IsEQP==null?false:Convert.ToBoolean(product.IsEQP),
                Material=product.Material,
                Size=product.Size,
                ImprintType=product.ImprintType,
                InkColor=product.InkColor,
                ImprintArea=product.ImprintArea,
                MultiColorImprintAvailable=product.MultiColorImprintAvailable,
                PriceIncludes=product.PriceIncludes,
                NormalProductionDays=product.NormalProductionDays,
                RushProductionDays=product.RushProductionDays
            };

            var applied = product.AppliedDiscounts.Where(x=>x.DiscountType==Core.Domain.Discounts.DiscountType.AssignedToSkus && x.UsePercentage==false);
            model.DiscountRanges = (from a in applied
                                    select new ProductDetailsModel.DiscountRange
                                    {
                                        Discount = a.Name,
                                        DiscountID = a.Id,
