...
namespace Nop.Web.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        public virtual OrderDetailsModel PrepareOrderDetailsModel(Order order)
        {
            ...
            model.IsSampleRequest = order.PaymentStatusId == (int)PaymentStatus.SampleRequest;
            //shipping info
            ...
            if (order.ShippingStatus != ShippingStatus.ShippingNotRequired)
            {
                ...
                foreach (var shipment in shipments)
                {
                    ...
                    if (!String.IsNullOrEmpty(shipment.TrackingNumber))
                    {
                        ...
                        {
                            shipmentModel.TrackingNumberUrl = shipmentTracker.GetUrl(shipment.TrackingNumber);
                        }
                    }
            ...
            foreach (var orderItem in orderItems)
            {
                var orderItemModel = new OrderDetailsModel.OrderItemModel
                {
                    ...
                    pictureid = Convert.ToInt32(orderItem.PictureId),
                    SetupFee = _priceFormatter.FormatPrice(_currencyService.ConvertCurrency(Convert.ToDecimal(orderItem.SetupFee), order.CurrencyRate))
                };
                //rental info
                ...
                //
                int lastorderid = 0;
                int.TryParse(_localizationService.GetResource("Order.LastOrderid"), out lastorderid);
                ...
                if (order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax)
                {
                    ...
                    if (order.Id <= lastorderid && orderItem.UnitPriceInclTax < 0)
                        orderItemModel.UnitPrice = "-" + orderItemModel.UnitPrice;
                    if (order.Id <= lastorderid && orderItem.PriceInclTax < 0)
                        orderItemModel.SubTotal = "-" + orderItemModel.SubTotal;
                ...
                else
                {
                    ...
                    if (order.Id <= lastorderid && orderItem.UnitPriceExclTax < 0)
                        orderItemModel.UnitPrice = "-" + orderItemModel.UnitPrice;
                    if (order.Id <= lastorderid && orderItem.PriceExclTax < 0)
                        orderItemModel.SubTotal = "-" + orderItemModel.SubTotal;
                }
