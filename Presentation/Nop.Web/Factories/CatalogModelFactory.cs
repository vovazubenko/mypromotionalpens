...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        protected virtual dynamic PrepareCategoryBannerModel(Category category, bool isAssociatedProduct = false)
        {
            if (category == null)
            ...
            var categoryName = category.GetLocalized(x => x.Name);

            var categoryBanners = _categoryService.GetCategoryBannerByCategoryId(category.Id);

            //all pictures
            var pictureModels = new List<CategoryBannerModel>();
            foreach (var categoryBanner in categoryBanners)
            {
                var picture = categoryBanner.Banner;
                if (picture != null)
                {
                    var pictureModel = new CategoryBannerModel
                    {
                        ...
                        Title = categoryName,
                        AlternateText = categoryName,
                    };
                    //"title" attribute
                    pictureModel.Title = !string.IsNullOrEmpty(picture.TitleAttribute) ?
                        picture.TitleAttribute : categoryName;
                    //string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat.Details"), categoryName);
                    //"alt" attribute
                    pictureModel.AlternateText = !string.IsNullOrEmpty(picture.AltAttribute) ?
                        picture.AltAttribute : categoryName;
                    //string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), categoryName);

                    pictureModel.BannerNumber = categoryBanner.BannerNumber;
                    pictureModel.BannerUrl = categoryBanner.BannerUrl;
                    pictureModel.DisplayOrder = categoryBanner.DisplayOrder;
                    pictureModels.Add(pictureModel);
                }
            }

            return new { PictureModels = pictureModels };
        }
        #endregion
        ...
        public virtual CategoryModel PrepareCategoryModel(Category category, CatalogPagingFilteringModel command)
        {
            ...
            var model = new CategoryModel
            {
                ...
                //FooterContent2 = category.FooterContent2,
                //FooterTitle3 = category.FooterTitle3,
                //FooterContent3 = category.FooterContent3,
            };

            //if (!string.IsNullOrEmpty(category.FooterTitle1) || !string.IsNullOrEmpty(category.FooterContent1))
            //    model.FooterContent.Add(category.FooterTitle1, category.FooterContent1);
            //if (!string.IsNullOrEmpty(category.FooterTitle2) || !string.IsNullOrEmpty(category.FooterContent2))
            //    model.FooterContent.Add(category.FooterTitle2, category.FooterContent2);
            //if (!string.IsNullOrEmpty(category.FooterTitle3) || !string.IsNullOrEmpty(category.FooterContent3))
            //    model.FooterContent.Add(category.FooterTitle3, category.FooterContent3);
            ...
            var pictureModels = PrepareCategoryBannerModel(category);
            model.CategoryBannerModels = pictureModels.PictureModels;
            //price ranges
