    ...
    public partial class CommonController : BasePublicController
    {
        ...
        public virtual ActionResult ContactUsSend(ContactUsModel model, bool captchaValid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //#region Insert contact into crm
                //try
                //{
                //    var _zohoCRMSetting = EngineContext.Current.Resolve<ZohoCRMSetting>();
                //    var _logger = EngineContext.Current.Resolve<ILogger>();

                //    ZohoCRMAPI zohoApi = new ZohoCRMAPI(_zohoCRMSetting, _logger, _localizationService);
                //    string xmlData = "<Contacts>↵<row no='1'>↵<FL val='First Name'></FL>↵<FL val='Last Name'>" + model.FullName + "</FL>↵<FL val='Email'>" + model.Email + "</FL>↵<FL val='Department'></FL>↵<FL val='Phone'></FL>↵<FL val='Fax'></FL>↵<FL val='Mobile'></FL>↵<FL val='Description'>" + model.Enquiry + "</FL>↵</row>↵</Contacts>";
                //    var result = zohoApi.ZohoCrmInsertContact(xmlData);
                //    if (string.IsNullOrWhiteSpace(result)) {
                //        model.SuccessfullySent = false;
                //        model.Result = _localizationService.GetResource("ContactUs.YourEnquiryHasBeenSentError");
                //        return View(model);
                //    }
                //}
                //catch (Exception ex)
                ...
                //    return View(model);
                //}
                ////var res = ZohoCrmXml.DeserializeXml<ZohoContactResponse>(result);
                //#endregion

                #region ZohodeskIntegration

                try
                {
                    var _zohoDeskApi = EngineContext.Current.Resolve<IZohoDeskApi>();
                    if (!string.IsNullOrEmpty(_zohoDeskApi.CreateTickets(model.FullName, model.Email, model.Subject, model.Enquiry,"")))
                    {
                        //model.SuccessfullySent = false;
                        //model.Result = _localizationService.GetResource("ContactUs.YourEnquiryHasBeenSentError");
                        //return View(model);
        ...
        public virtual ActionResult SitemapXml(int? id)
        {
            if (!_commonSettings.SitemapEnabled)
                return RedirectToRoute("HomePage");
            var _scheduleTaskService = EngineContext.Current.Resolve<IScheduleTaskService>();
            var scheduleTask = _scheduleTaskService.GetTaskByType(typeof(SiteMapGenerateTask).FullName);
            if (scheduleTask == null)
            {
                scheduleTask = new ScheduleTask();
                scheduleTask.Name = "SiteMap";
                scheduleTask.Seconds = 86400;
                scheduleTask.Type = typeof(SiteMapGenerateTask).FullName;
