...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
        ...
        [ValidateInput(false)]
        [FormValueRequired("reorder-payment")]
        public virtual ActionResult ReOrderPayment(int orderId,string ReorderComment)
        {
            ...
            var ReOrderStoreOwnerNotificationQueuedEmailId = _workflowMessageService
                  .ReOrderStoreOwnerNotification(order, order.CustomerLanguageId, ReorderComment);
            var ReOrderCustomerNotificationQueuedEmailId = _workflowMessageService
                  .ReOrderCustomerNotification(order, order.CustomerLanguageId, ReorderComment);
            if (ReOrderStoreOwnerNotificationQueuedEmailId > 0 && ReOrderCustomerNotificationQueuedEmailId>0)
            {
                SuccessNotification(_localizationService.GetResource("Order.Reorder.Success"), true);
            }
            else {
                ErrorNotification(_localizationService.GetResource("Order.Reorder.Error"), true);
            }
            return RedirectToRoute("OrderDetails", new { orderId = orderId });
        }
        #endregion
