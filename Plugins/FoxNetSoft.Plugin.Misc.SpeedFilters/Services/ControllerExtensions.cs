            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                if (preparePictureModel)
                {
                    ...
                    {
                        if (pictureService.GetPicturesByProductId(product.Id, 1).Count > 0)
                        {
                            var picture = pictureService.GetPicturesByProductId(product.Id, 1).FirstOrDefault();
                            var ListingUrl = "";
                            if (pictureService.GetPictureUrl(picture, pictureSize).Contains("Content/Images/ProductImages"))
                            {
                                ListingUrl = (pictureService.GetPictureUrl(picture, pictureSize)).Replace(".jpg", "_List.jpg").Replace(".JPG", "_List.JPG").Replace(".jpeg", "_List.jpeg").Replace(".JPEG", "_List.JPEG");
                            }
                            else
                            {
                                ListingUrl = pictureService.GetPictureUrl(picture, pictureSize);
                            }
                            var pictureModel = new PictureModel
                            {
                                ImageUrl = ListingUrl,
                                FullSizeImageUrl = ListingUrl//(pictureService.GetPictureServerUrl(picture)).Replace(".jpg", "_List.jpg").Replace(".JPG", "_List.JPG").Replace(".jpeg", "_List.jpeg").Replace(".JPEG", "_List.JPEG")
                            };
                            //"title" attribute
                        ...
                        {
                            var ListingUrl = "";
                            if (pictureService.GetDefaultPictureUrl().Contains("Content/Images/ProductImages"))
                            {
                                ListingUrl = (pictureService.GetDefaultPictureUrl()).Replace(".jpg", "_List.jpg").Replace(".JPG", "_List.JPG").Replace(".jpeg", "_List.jpeg").Replace(".JPEG", "_List.JPEG");
                            }
                            else
                            {
                                ListingUrl = pictureService.GetDefaultPictureUrl();
                            }
                            var pictureModel = new PictureModel
                            {
                                ImageUrl = ListingUrl, //(pictureService.GetDefaultPictureUrl()).Replace(".jpg", "_List.jpg").Replace(".JPG", "_List.JPG").Replace(".jpeg", "_List.jpeg").Replace(".JPEG", "_List.JPEG"),
                                FullSizeImageUrl = ListingUrl//(pictureService.GetDefaultPictureUrl()).Replace(".jpg", "_List.jpg").Replace(".JPG", "_List.JPG").Replace(".jpeg", "_List.jpeg").Replace(".JPEG", "_List.JPEG")
                            };
                            //"title" attribute
                            pictureModel.Title = "";
                            //"alt" attribute
