...
namespace Nop.Plugin.Shipping.UPS.Controllers
{
    ...
    public class ShippingUPSController : BasePluginController
    {
        ...
        public ActionResult Configure()
        {
            ...
            model.IsLiveRates = _upsSettings.IsLiveRates;
            model.isusafreeshipping = _upsSettings.isusafreeshipping;
            model.iscontigousfreeshipping = _upsSettings.iscontigousfreeshipping;
            ...
            var Rates = _shippingSerive.GetShippingRates(false, "UPS").ToList();
            foreach (string service in UPSServices.Services)
            {
                string serviceId = UPSServices.GetServiceId(service);
                ...
                if (!String.IsNullOrEmpty(serviceId))
                {
                    model.ShippingRate.Add(Rates.Where(x => x.CarrierId == serviceId).FirstOrDefault() == null ? "" : Rates.Where(x => x.CarrierId == serviceId).FirstOrDefault().Rate.ToString());
                }
                else {
                    model.ShippingRate.Add("");
                }
        ...
        [HttpPost]
        [ChildActionOnly]
        public ActionResult Configure(UPSShippingModel model, FormCollection form)
        {
            var _shippingSerive = EngineContext.Current.Resolve<IShippingService>();
            ...
            _upsSettings.IsLiveRates = model.IsLiveRates;
            _upsSettings.isusafreeshipping = model.isusafreeshipping;
            _upsSettings.iscontigousfreeshipping = model.iscontigousfreeshipping;
            ...
            if (model.CheckedCarrierServices != null)
            {
                ...
                _shippingSerive.DeleteAllShippingRates(false, "UPS");
                _shippingSerive.InsertAllShippingRates(dictDomestic, false, dictDomesticCarrierName, "UPS");
            }
