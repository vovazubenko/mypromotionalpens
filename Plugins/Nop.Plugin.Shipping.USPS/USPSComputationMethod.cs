...
namespace Nop.Plugin.Shipping.USPS
{
    ...
    public class USPSComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        public GetShippingOptionResponse GetShippingOptions(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            //getShippingOptionRequest.Items.ToList().Select(c => { c.ShoppingCartItem.Product.IsFreeShipping = false; return c; }).ToList();
            ...
            {
                if (shippingOptions.Any() && _uspsSettings.IsLiveRates==true)
                {
                    foreach (var shippingOption in shippingOptions)
                    {
                        ...
                        //if (_uspsSettings.isusafreeshipping==true && getShippingOptionRequest.ShippingAddress.Country.ThreeLetterIsoCode.ToLower() == "usa")
                        //{
            ...
            if (!response.ShippingOptions.Any())
            {
                ...
                var availableCarrierShippingOptions = _shippingService.GetShippingRates(isDomestic, "USPS").ToList();
                foreach (var shippingOption in availableCarrierShippingOptions)
                {
                    ...
                    if (!shippingOption.CarrierName.ToLower().StartsWith("usps"))
                        option.Name = string.Format("USPS {0}", shippingOption.CarrierName);
                    ...
                    option.Rate += _uspsSettings.AdditionalHandlingCharge;
        ...
        {
            //get { return null; }
            get { return new USPSShipmentTracker(_uspsSettings); }
        }
