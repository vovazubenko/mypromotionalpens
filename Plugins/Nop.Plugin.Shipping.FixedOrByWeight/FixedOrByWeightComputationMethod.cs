...
namespace Nop.Plugin.Shipping.FixedOrByWeight
{
    ...
    public class FixedOrByWeightComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        public GetShippingOptionResponse GetShippingOptions(GetShippingOptionRequest getShippingOptionRequest)
        {
            ...
            if (_fixedOrByWeightSettings.ShippingByWeightEnabled)
            {
                ...
                foreach (var shippingMethod in shippingMethods)
                {
                    ...
                    if (shippingMethod.Name.ToLower().Contains("free"))
                    {
                        if (ISFreeShipEnable(getShippingOptionRequest))
                        {
                            response.ShippingOptions.Add(shippingOption);
                        }
                    }
                    else { response.ShippingOptions.Add(shippingOption); }
        ...
        protected bool ISFreeShipEnable(GetShippingOptionRequest getShippingOptionRequest)
        {
            bool result = false;
            ...
            if (_shippingSettings.FreeShippingOverXEnabled)
            {
                ...
                _orderTotalCalculationService.GetShoppingCartSubTotal(getShippingOptionRequest.Items.Select(x=>x.ShoppingCartItem).ToList(), _shippingSettings.FreeShippingOverXIncludingTax, out discountAmount,
            ...
            if (_shippingSettings.isusafreeshipping == true && getShippingOptionRequest.ShippingAddress.Country.ThreeLetterIsoCode.ToLower() == "usa")
            {
                if (_shippingSettings.iscontigousfreeshipping == true)
                {
                    result = true;
                }
                else {
                    if (!IsContigousState(getShippingOptionRequest))
                    {
                        ...
                        if (!getShippingOptionRequest.Items.Where(x => x.ShoppingCartItem.IsFreeShipping == false).Any())
                        {
                            result = true;
                        }
                    }
