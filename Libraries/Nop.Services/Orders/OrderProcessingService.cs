...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected class PlaceOrderContainter
        {
            ...
            public DateTime POInHandDate { get; set; }
            public DateTime ShipByDate { get; set; }
            public DateTime POSentDate { get; set; }
            public DateTime CustInHandDate { get; set; }
        ...
        protected virtual PlaceOrderContainter PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest, bool isSampleRequest)
        {
            ...
            if (isSampleRequest == true)
            {
                details.ShippingAddress = (Address)details.Customer.BillingAddress.Clone();
            ...
            {
                details.Cart = details.Customer.ShoppingCartItems.Where(sci => sci.ShoppingCartType == ShoppingCartType.SampleCart)
                    .LimitPerStore(processPaymentRequest.StoreId).ToList();
                //details.Cart.ToList().ForEach(c => { c.Product.Price = 0; });
            }
            ...
            if (!isSampleRequest && !ValidateMinOrderSubtotalAmount(details.Cart))
            {
            ...
            if (!isSampleRequest && !ValidateMinOrderTotalAmount(details.Cart))
            {
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainter details)
        {
            var order = new Order
            {
                ...
                OrderNumber = processPaymentRequest.OrderNumber
            };
        ...
        protected virtual void SendNotificationsAndSaveNotes(Order order)
        {
            ...
            {
                if (order.PaymentStatus == PaymentStatus.SampleRequest)
                {
                    if (_workContext.CurrentCustomer.GetAttribute<bool?>("ImpersonateSampleCartSendMail") == true)
                        sendMail = true;
                }
                else {
                    if (_workContext.CurrentCustomer.GetAttribute<bool?>("ImpersonateSendMail") == true)
                        sendMail = true;
                }
            }
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest, bool isSampleRequest = false)
        {
            ...
            try
            {
                var lastOrder = _orderService.GetLastOrder();
                processPaymentRequest.OrderNumber = _workContext.CurrentCustomer.Id + "-" + (lastOrder != null ? (lastOrder.Id + 1) : 1);

                if (processPaymentRequest.OrderGuid == Guid.Empty)
                    processPaymentRequest.OrderGuid = Guid.NewGuid();

                //prepare order details
                var details = PreparePlaceOrderDetails(processPaymentRequest, isSampleRequest);
                ...
                if (!skipPaymentWorkflow && isSampleRequest == false)
                {
                    ...
                    processPaymentResult = new ProcessPaymentResult { NewPaymentStatus = PaymentStatus.SampleRequest };
                else
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    order.CustomerProfileId = processPaymentResult.CustomerProfileId;
                    order.CustomerPaymentProfileId = processPaymentResult.CustomerPaymentProfileId;
                    if (!string.IsNullOrEmpty(order.PaymentMethodSystemName) && order.PaymentMethodSystemName.ToLower() != "Payments.CheckMoneyOrder".ToLower())
                    {
                    ...
                    foreach (var sc in details.Cart)
                    {
                        ...
                        var orderItem = new OrderItem
                        {
                            ...
                            OriginalProductCost = isSampleRequest ? 0 : _priceCalculationService.GetProductCost(sc.Product, sc.AttributesXml),
                            ...
                            PictureId = sc.PictureId,
                            RentalStartDateUtc = sc.RentalStartDateUtc,
                            RentalEndDateUtc = sc.RentalEndDateUtc,
                            SetupFee = isSampleRequest ? 0 : sc.SetupFee
                    ...
                    if (order.PaymentStatus == PaymentStatus.SampleRequest)
                    {
                        _genericAttributeService.SaveAttribute<bool?>(order.Customer, "ImpersonateSampleCartSendMail", null);
                    }
                    else {
                        _genericAttributeService.SaveAttribute<bool?>(order.Customer, "ImpersonateSendMail", null);
                    }
                    //reset checkout data
                    if (!isSampleRequest)
                    {
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            try
            {
                ...
                request.AmountToRefund = order.OrderBaseAmount;
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            decimal amountToRefund = order.OrderBaseAmount;
        ...
        public virtual void ReOrder(Order order)
        {
            ...
            foreach (var orderItem in order.OrderItems)
            {
                _shoppingCartService.AddToCart(order.Customer, orderItem.Product,
                    ...
                    orderItem.Quantity, false, orderItem.PictureId == null ? 0 : Convert.ToInt32(orderItem.PictureId));
            }


            //code for duplicate order attribute
            var _checkoutAttributeParser = EngineContext.Current.Resolve<ICheckoutAttributeParser>();
            var checkoutAttr = EngineContext.Current.Resolve<ICheckoutAttributeService>();
            CheckoutAttribute ck = checkoutAttr.GetAllCheckoutAttributes().Where(x => x.Name.Contains("Duplicate Order Number")).FirstOrDefault();
            if (ck != null)
            {
                order.CheckoutAttributesXml = _checkoutAttributeParser.RemoveCheckoutAttribute(order.CheckoutAttributesXml, ck);

                order.CheckoutAttributesXml = _checkoutAttributeParser.AddCheckoutAttribute(order.CheckoutAttributesXml
                    , ck, Convert.ToString(order.Id));
            }
        ...
        public virtual bool IsPaymentWorkflowRequired(IList<ShoppingCartItem> cart, bool? useRewardPoints = null)
        {
            ...
            if (shoppingCartTotalBase.HasValue && shoppingCartTotalBase.Value == decimal.Zero)
                result = false;
            if (cart.Count > 0 && cart[0].ShoppingCartType == ShoppingCartType.SampleCart)
                return false;
            return result;
        }
