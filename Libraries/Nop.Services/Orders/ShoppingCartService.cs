...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        public ShoppingCartService(IRepository<ShoppingCartItem> sciRepository,
            ...
            IDateTimeHelper dateTimeHelper, IPictureService pictureService)
        {
            ...
            bool ensureOnlyActiveCheckoutAttributes = false)
        {
            ...
            if (shoppingCartItem.PictureId != null && shoppingCartItem.PictureId > 0)
            {
                _pictureService.DeletePicture(shoppingCartItem.picture);
            }
            ...
            int quantity)
        {
            ...
            if (quantity < product.OrderMinimumQuantity && shoppingCartType !=ShoppingCartType.SampleCart)
            {
        ...
        public virtual IList<string> AddToCart(Customer customer, Product product,
            ...
            decimal customerEnteredPrice = decimal.Zero,
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null,
            int quantity = 1, bool automaticallyAddRequiredProductsIfEnabled = true,int pictureid=0)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            if (shoppingCartItem != null)
            {
                ...
                if (shoppingCartType == ShoppingCartType.SampleCart)
                    newQuantity = 1;
            ...
            checkoutAttributesXml= _checkoutAttributeParser.RemoveCheckoutAttribute(checkoutAttributesXml, _checkoutAttributeService.GetAllCheckoutAttributes().Where(x => x.Name.Contains("Duplicate Order Number")).FirstOrDefault());
            _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer, SystemCustomerAttributeNames.CheckoutAttributes, checkoutAttributesXml, _storeContext.CurrentStore.Id);
        ...
        public virtual void UpdateShoppingCartItemImamge(Customer customer,
            int shoppingCartItemId, int PictureId)
        {

            Nop.Core.Domain.Media.Picture pic = null;
            int lastPicID = 0;
            ...
            if (shoppingCartItem != null)
            {
                lastPicID = shoppingCartItem.PictureId ?? 0;
                //if everything is OK, then update a shopping cart item
                if (PictureId <= 0)
                    shoppingCartItem.PictureId = null;
                else
                    shoppingCartItem.PictureId = PictureId;
                //event notification
                _eventPublisher.EntityUpdated(shoppingCartItem);
                if (lastPicID > 0 && lastPicID != PictureId)
                {
                    pic =  _pictureService.GetPictureById(lastPicID);
        ...
        public virtual void MigrateShoppingCart(Customer fromCustomer, Customer toCustomer, bool includeCouponCodes)
        {
            ...
            for (int i = 0; i < fromCart.Count; i++)
            {
                ...
                AddToCart(toCustomer, sci.Product, sci.ShoppingCartType, sci.StoreId,
                    ...
                    sci.RentalStartDateUtc, sci.RentalEndDateUtc, sci.Quantity, false,Convert.ToInt32(sci.PictureId));
            }
            for (int i = 0; i < fromCart.Count; i++)
            {
                var sci = fromCart[i];
            ...
            // save checkout attribute

            var attributeXml = fromCustomer.GetAttribute<string>(SystemCustomerAttributeNames.CheckoutAttributes,
             _genericAttributeService, _storeContext.CurrentStore.Id);

            _genericAttributeService.SaveAttribute(toCustomer, SystemCustomerAttributeNames.CheckoutAttributes,attributeXml, _storeContext.CurrentStore.Id);
