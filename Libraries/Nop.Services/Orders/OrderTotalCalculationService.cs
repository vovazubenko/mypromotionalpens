...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        protected virtual decimal GetOrderSubtotalDiscount(Customer customer,
            decimal orderSubTotal, out List<DiscountForCaching> appliedDiscounts,Order order=null)
        {
            ...
            if (!allowedDiscounts.Any())
            {
                if (order != null)
                {
                    if (order.DiscountUsageHistory.Any())
                    {
                        foreach (var discount in allDiscounts)
                        {
                            if (order.DiscountUsageHistory.Any(x => x.DiscountId == discount.Id))
                            {
                                allowedDiscounts.Add(discount);
                            }
                        }
        ...
        /// <param name="appliedDiscounts">Applied discounts</param>
        /// <returns>Order discount</returns>
        protected virtual decimal GetOrderTotalDiscount(Customer customer, decimal orderTotal, out List<DiscountForCaching> appliedDiscounts,Order order=null)
        {
            appliedDiscounts = new List<DiscountForCaching>();
            decimal discountAmount = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
        ...
        public virtual void UpdateOrderTotals(UpdateOrderParameters updateOrderParameters, IList<ShoppingCartItem> restoredCart)
        {
            ...
            var discountAmountExclTax = GetOrderSubtotalDiscount(customer, subTotalExclTax, out subTotalDiscounts,updatedOrder);
            //SP:
            if (discountAmountExclTax <= 0) {
                discountAmountExclTax = updatedOrder.OrderSubTotalDiscountExclTax;
            }
            ...
            var discountAmountTotal = GetOrderTotalDiscount(customer, total, out orderAppliedDiscounts,updatedOrder);

            //SP:
            if (discountAmountTotal <= 0) {
                discountAmountTotal = updatedOrder.OrderDiscount;
            }
