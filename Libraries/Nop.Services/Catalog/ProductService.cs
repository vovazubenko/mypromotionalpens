...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// <returns>Products</returns>
        public virtual IList<Product> GetProductsByNames(string[] productNames)
        {
            if (productNames == null || productNames.Length == 0)
                return new List<Product>();

            var query = from p in _productRepository.Table
                where productNames.Contains(p.Name) && !p.Deleted
                select p;
            var products = query.ToList();
        ...
        public virtual IPagedList<Product> SearchProducts(
            ...
            ProductSortingEnum orderBy = ProductSortingEnum.PriceAsc,
        ...
        public virtual IPagedList<Product> SearchProducts(
            ...
            ProductSortingEnum orderBy = ProductSortingEnum.PriceAsc,
            bool showHidden = false,
            bool? overridePublished = null,DateTime? fromUtc=null, DateTime? toUtc = null, bool? IsASI = false)
        {
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                var pIsASI = _dataProvider.GetParameter();
                pIsASI.ParameterName = "IsASI";
                if(IsASI.HasValue)
                    pIsASI.Value = IsASI;
                else
                    pIsASI.Value = DBNull.Value;
                pIsASI.DbType = DbType.Boolean;

                var pfromUtc = _dataProvider.GetParameter();
                pfromUtc.ParameterName = "fromUtc";
                if(fromUtc.HasValue)
                    pfromUtc.Value = fromUtc;
                else
                    pfromUtc.Value = DBNull.Value;
                pfromUtc.DbType = DbType.DateTime;

                var ptoUtc = _dataProvider.GetParameter();
                ptoUtc.ParameterName = "toUtc";
                if(toUtc.HasValue)
                    ptoUtc.Value = toUtc;
                else
                    ptoUtc.Value = DBNull.Value;
                ...
                var products = _dbContext.ExecuteStoredProcedureList<Product>(
                    ...
                    pFilterableSpecificationAttributeOptionIds,pIsASI,pfromUtc, ptoUtc,
                    pTotalRecords);
            ...
            else
            {
                ...
                {
                    //published only
                    query = query.Where(p => p.MarkAsNewASI);
                }
        ...
        #region Get Latest Reviews
        public virtual IList<ProductReview> GetLatestReviews() {
            var query = _productReviewRepository.Table.Where(x=>x.IsApproved==true && x.Product!=null);
            query = query.Where(x=>x.Product.ApprovedTotalReviews>0).OrderByDescending(x => x.Id).ThenByDescending(x => x.ProductId).Take(3);
            var productReviews = query.ToList();
            return productReviews;
        }
        #endregion
