...
namespace Nop.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
        ...
        protected virtual decimal GetDiscountAmount(Product product,
            ...
            out List<DiscountForCaching> appliedDiscounts, int Quantity = 0)
        {
            if (product == null)
            ...
            appliedDiscounts = allowedDiscounts.GetPreferredDiscount(productPriceWithoutDiscount, out appliedDiscountAmount, Quantity,Convert.ToBoolean(product.IsEQP));
            return appliedDiscountAmount;
        }
            ...
            out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            var cachedPrice = _cacheManager.Get(cacheKey, cacheTime, () =>
            {
                ...
                if (includeDiscounts)
                {
                    ...
                    decimal tmpDiscountAmount = GetDiscountAmount(product, customer, price, out tmpAppliedDiscounts,quantity);
            ...
            out int? maximumDiscountQty)
        {
            ...
            subTotal = subTotal + Convert.ToDecimal(shoppingCartItem.SetupFee);
            return subTotal;
        }
        ...
        #region Discount Validate
        public IList<DiscountForCaching> GetAllowedDiscountsTest(Product product, Customer customer, string[] couponCodesToValidate)
        {
            var allowedDiscounts = new List<DiscountForCaching>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;
            ...
            //discounts applied to products
            foreach (var discount in GetAllowedDiscountsAppliedToProductTest(product, customer, couponCodesToValidate))
                if (!allowedDiscounts.ContainsDiscount(discount))
                    allowedDiscounts.Add(discount);

            //discounts applied to categories
            foreach (var discount in GetAllowedDiscountsAppliedToManufacturersTest(product, customer, couponCodesToValidate))
                if (!allowedDiscounts.ContainsDiscount(discount))
                    allowedDiscounts.Add(discount);

            //discounts applied to manufacturers
            foreach (var discount in GetAllowedDiscountsAppliedToCategoriesTest(product, customer, couponCodesToValidate))
                if (!allowedDiscounts.ContainsDiscount(discount))
                    allowedDiscounts.Add(discount);

            return allowedDiscounts;
        }
        protected virtual IList<DiscountForCaching> GetAllowedDiscountsAppliedToProductTest(Product product, Customer customer, string[] couponCodesToValidate)
        {
            var allowedDiscounts = new List<DiscountForCaching>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;
            ...
            if (product.HasDiscountsApplied)
            {
                ...
                foreach (var discount in product.AppliedDiscounts)
                {
                    if (_discountService.ValidateDiscount(discount, customer, couponCodesToValidate).IsValid &&
                        discount.DiscountType == DiscountType.AssignedToSkus)
                        allowedDiscounts.Add(discount.MapDiscount());
                }
            }
        ...
        protected virtual IList<DiscountForCaching> GetAllowedDiscountsAppliedToManufacturersTest(Product product, Customer customer, string[] couponCodesToValidate)
        {
            var allowedDiscounts = new List<DiscountForCaching>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;
        ...
        protected virtual IList<DiscountForCaching> GetAllowedDiscountsAppliedToCategoriesTest(Product product, Customer customer, string[] couponCodesToValidate)
        {
            var allowedDiscounts = new List<DiscountForCaching>();
            if (_catalogSettings.IgnoreDiscounts)
                return allowedDiscounts;
