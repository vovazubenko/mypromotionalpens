...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        #region Custom Shipping Rate
        public IList<ShippingRates> GetShippingRates(bool isDomestic,string ShippingCarrier) {
           var _shippingRatesRepository = EngineContext.Current.Resolve<IRepository<ShippingRates>>();
            return _shippingRatesRepository.Table.ToList().Where(x=>x.IsDomestic==isDomestic && x.ShippingCarrier== ShippingCarrier).ToList();
        }
        public void InsertAllShippingRates(Dictionary<string,decimal> dictShippingRates,bool IsDomestic, Dictionary<string,string> dictCarrierName,string ShippingCarrier)
        {
            var _shippingRatesRepository = EngineContext.Current.Resolve<IRepository<ShippingRates>>();
            foreach (var item in dictShippingRates) {
                ShippingRates model = new ShippingRates();
                model.CarrierId = item.Key;
                model.Rate = item.Value;
                model.IsDomestic = IsDomestic;
                model.CarrierName =Convert.ToString(dictCarrierName[item.Key]);
                model.ShippingCarrier = ShippingCarrier;
                _shippingRatesRepository.Insert(model);
            }
            
        }
        public void DeleteAllShippingRates(bool isDomestic,string ShippingCarrier)
        {
            var _shippingRatesRepository = EngineContext.Current.Resolve<IRepository<ShippingRates>>();
            _shippingRatesRepository.Table.Where(x=>x.IsDomestic==isDomestic && x.ShippingCarrier== ShippingCarrier).ToList().ForEach(x => _shippingRatesRepository.Delete(x));
        }

        public IList<ShippingRates> GetAllShippingRates() {
            var _shippingRatesRepository = EngineContext.Current.Resolve<IRepository<ShippingRates>>();
            return _shippingRatesRepository.Table.ToList();
        }
        public void DeleteShippingRatesByCarrierNameID(string ShippingCarrier,string Carrierid)
        {
            var _shippingRatesRepository = EngineContext.Current.Resolve<IRepository<ShippingRates>>();
            _shippingRatesRepository.Table.Where(x =>x.ShippingCarrier == ShippingCarrier && x.CarrierId== Carrierid).ToList().ForEach(x => _shippingRatesRepository.Delete(x));
        }
        #endregion
    }
}
