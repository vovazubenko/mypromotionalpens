...
namespace Nop.Services.Messages
{
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public virtual int SendOrderPlacedVendorNotification(Order order, Vendor vendor, int languageId)
        {
            ...
            languageId = EnsureLanguageIsActive(languageId, store.Id);
            string templateName = MessageTemplateSystemNames.OrderPlacedVendorNotification;
            if (order.PaymentStatusId == (int)Nop.Core.Domain.Payments.PaymentStatus.SampleRequest)
            {
                templateName = "SampleRequestPlaced.VendorNotification";
            }
            var messageTemplate = GetActiveMessageTemplate(templateName, store.Id);
            if (messageTemplate == null)
                return 0;
        ...
        public virtual int SendOrderPlacedStoreOwnerNotification(Order order, int languageId)
        {
            ...
            var store = _storeService.GetStoreById(order.StoreId) ?? _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);
            string templateName = MessageTemplateSystemNames.OrderPlacedStoreOwnerNotification;
            if (order.PaymentStatusId == (int)Nop.Core.Domain.Payments.PaymentStatus.SampleRequest)
            {
                templateName = "SampleRequestPlaced.StoreOwnerNotification";
            }
            var messageTemplate = GetActiveMessageTemplate(templateName, store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            ...
            var store = _storeService.GetStoreById(order.StoreId) ?? _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);
            string templateName = MessageTemplateSystemNames.OrderPlacedCustomerNotification;
            if (order.PaymentStatusId == (int)Nop.Core.Domain.Payments.PaymentStatus.SampleRequest)
            {
                templateName = "SampleRequestPlaced.CustomerNotification";
            }
            var messageTemplate = GetActiveMessageTemplate(templateName, store.Id);
            if (messageTemplate == null)
                return 0;
        ...
        #region Product Sample-Quote-Proof request mail
        public virtual int SendProductProofRequestFormMessage(int languageId, string senderEmail,
           string senderName, string subject, string body, dynamic Model)
        {
            ...
            var messageTemplate = GetActiveMessageTemplate("Product.ProofRequestForm", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            tokens.Add(new Token("ProofRequest.FullName", Model.FullName, true));
            tokens.Add(new Token("ProofRequest.Company", Model.Company, true));
            tokens.Add(new Token("ProofRequest.Address1", Model.Address1, true));
            tokens.Add(new Token("ProofRequest.Address2", Model.Address2, true));
            tokens.Add(new Token("ProofRequest.City", Model.City, true));
            tokens.Add(new Token("ProofRequest.State", Model.State, true));
            tokens.Add(new Token("ProofRequest.Country", Model.Country, true));
            tokens.Add(new Token("ProofRequest.Zip", Model.Zip, true));
            tokens.Add(new Token("ProofRequest.Email", Model.EmailAddress, true));
            tokens.Add(new Token("ProofRequest.Phone", Model.PhoneNumber, true));

            tokens.Add(new Token("ProofRequest.ProductCode", Model.ProductCode, true));
            tokens.Add(new Token("ProofRequest.ProductName", Model.ProductName, true));
            tokens.Add(new Token("ProofRequest.ItemColor", Model.ItemColor, true));
            tokens.Add(new Token("ProofRequest.ImprintColor", Model.ImprintColor, true));
            tokens.Add(new Token("ProofRequest.ImprintText", Model.ImprintText, true));
            tokens.Add(new Token("ProofRequest.Comment", Model.Comment, true));

            if (!string.IsNullOrEmpty(Model.ProductPictureUrl))
            {
                string downloadLink = string.Format("<img src=\"{0}\" alt=\"\" />", Model.ProductPictureUrl);
                tokens.Add(new Token("ProofRequest.ProductPictureUrl", downloadLink, true));
            }
            else
            {
                tokens.Add(new Token("ProofRequest.ProductPictureUrl", "-", true));
            }

            if (!string.IsNullOrEmpty(Model.ArtUpload))
            {
                string downloadUrl = string.Format("{0}Download/GetFileUpload?downloadId={1}", store.Url, Model.ArtUpload);
                string downloadLink = string.Format("<a class=\"link\" href=\"{0}\">{1}</a>", downloadUrl, "Download");
                tokens.Add(new Token("ProofRequest.ArtUpload", downloadLink, true));
            }
            else
            {
                tokens.Add(new Token("ProofRequest.ArtUpload", "", true));
            }
        ...
        public virtual int SendProductQuoteRequestFormMessage(int languageId, string senderEmail,
          string senderName, string subject, string body, dynamic Model)
        {
            ...
            var messageTemplate = GetActiveMessageTemplate("Product.QuoteRequestForm", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            tokens.Add(new Token("QuoteRequest.FullName", Model.FullName, true));
            tokens.Add(new Token("QuoteRequest.Company", Model.Company, true));
            tokens.Add(new Token("QuoteRequest.Address1", Model.Address1, true));
            tokens.Add(new Token("QuoteRequest.Address2", Model.Address2, true));
            tokens.Add(new Token("QuoteRequest.City", Model.City, true));
            tokens.Add(new Token("QuoteRequest.State", Model.State, true));
            tokens.Add(new Token("QuoteRequest.Country", Model.Country, true));
            tokens.Add(new Token("QuoteRequest.Zip", Model.Zip, true));
            tokens.Add(new Token("QuoteRequest.Email", Model.EmailAddress, true));
            tokens.Add(new Token("QuoteRequest.Phone", Model.PhoneNumber, true));

            tokens.Add(new Token("QuoteRequest.ProductCode", Model.ProductCode, true));
            tokens.Add(new Token("QuoteRequest.ProductName", Model.ProductName, true));
            tokens.Add(new Token("QuoteRequest.ItemColor", Model.ItemColor, true));
            tokens.Add(new Token("QuoteRequest.ImprintColor", Model.ImprintColor, true));
            tokens.Add(new Token("QuoteRequest.ImprintText", Model.ImprintText, true));
            tokens.Add(new Token("QuoteRequest.Comment", Model.Comment, true));
            tokens.Add(new Token("QuoteRequest.Quantity", Model.Quantity, true));

            if (!string.IsNullOrEmpty(Model.ProductPictureUrl))
            {
                string downloadLink = string.Format("<img src=\"{0}\" alt=\"\" />", Model.ProductPictureUrl);
                tokens.Add(new Token("QuoteRequest.ProductPictureUrl", downloadLink, true));
            }
            else
            {
                tokens.Add(new Token("QuoteRequest.ProductPictureUrl", "-", true));
            }
            ...
            if (!string.IsNullOrEmpty(Model.ArtUpload))
            {
                string downloadUrl = string.Format("{0}Download/GetFileUpload?downloadId={1}", store.Url, Model.ArtUpload);
                string downloadLink = string.Format("<a class=\"link\" href=\"{0}\">{1}</a>", downloadUrl, "Download");
                tokens.Add(new Token("QuoteRequest.ArtUpload", downloadLink, true));
            }
            else
            {
                tokens.Add(new Token("QuoteRequest.ArtUpload", "", true));
            }
            tokens.Add(new Token("Store.Name", store.Name));
        ...
        public virtual int SendProductSampleRequestFormMessage(int languageId, string senderEmail,
          string senderName, string subject, string body, dynamic Model)
        {
            if (string.IsNullOrEmpty(body))
                throw new ArgumentNullException("Empty data");
            ...
            var messageTemplate = GetActiveMessageTemplate("Product.SampleRequestForm", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            tokens.Add(new Token("SampleRequest.FullName", Model.FullName, true));
            tokens.Add(new Token("SampleRequest.Company", Model.Company, true));
            tokens.Add(new Token("SampleRequest.Address1", Model.Address1, true));
            tokens.Add(new Token("SampleRequest.Address2", Model.Address2, true));
            tokens.Add(new Token("SampleRequest.City", Model.City, true));
            tokens.Add(new Token("SampleRequest.Country", Model.Country, true));
            tokens.Add(new Token("SampleRequest.State", Model.State, true));
            tokens.Add(new Token("SampleRequest.Zip", Model.Zip, true));
            tokens.Add(new Token("SampleRequest.Email", Model.EmailAddress, true));
            tokens.Add(new Token("SampleRequest.Phone", Model.PhoneNumber, true));
            tokens.Add(new Token("SampleRequest.ProductName", Model.ProductName, true));
            tokens.Add(new Token("SampleRequest.ProductCode", Model.ProductCode, true));
            tokens.Add(new Token("SampleRequest.ProductColor", Model.ProductColor, true));

            if (!string.IsNullOrEmpty(Model.ProductPictureUrl))
            {
                string downloadLink = string.Format("<img src=\"{0}\" alt=\"\" />", Model.ProductPictureUrl);
                tokens.Add(new Token("SampleRequest.ProductPictureUrl", downloadLink, true));
            }
            else
            {
                tokens.Add(new Token("SampleRequest.ProductPictureUrl", "-", true));
            }
        ...
        #region ProductAddedToCartNotificationToCustomer
        /// <summary>
        /// Sends an order placed notification to a customer
        /// </summary>
        /// <param name="order">Order instance</param>
        ...
        /// <param name="attachmentFileName">Attachment file name. If specified, then this file name will be sent to a recipient. Otherwise, "AttachmentFilePath" name will be used.</param>
        /// <returns>Queued email identifier</returns>
        public virtual int ProductAddedToCartNotificationToCustomer(List<ShoppingCartItem> item, int languageId,
            string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (!item.Any())
                throw new ArgumentNullException("Cart");

            var store = _storeService.GetStoreById(item.FirstOrDefault().StoreId) ?? _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplate("AddToCart.ImporsanateCustomerNotification", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddCartTokens(tokens, item, languageId);
            _messageTokenProvider.AddCustomerTokens(tokens, item.FirstOrDefault().Customer);

            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);

            var toEmail = item.FirstOrDefault().Customer.Email;
            var toName = string.Format("{0}", item.FirstOrDefault().Customer.Username);
        ...
        public virtual int SendPurchaseOrderNotification(PurchaseOrder po, string podate, string deliverydate, int languageId, string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (po == null)
                throw new ArgumentNullException("PurchaseOrder");

            var store = _storeService.GetStoreById(po.Order.StoreId) ?? _storeContext.CurrentStore;
            languageId = EnsureLanguageIsActive(languageId, store.Id);

            var messageTemplate = GetActiveMessageTemplate("PurchaseOrder.PurchaseOrderNotification", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            tokens.Add(new Token("Purchaseorder.Purchaseordernumber", po.PONumber));
            tokens.Add(new Token("Vendor.Name", po.VendorTitle));
            tokens.Add(new Token("Purchaseorder.PODate", podate));
            tokens.Add(new Token("Purchaseorder.PODeliverydate", deliverydate));

            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);

            var toEmail = po.VendorEmail;
            var toName = po.VendorTitle;
        ...
        public virtual int SendOrderInvoiceCustomerNotification(Order order, int languageId,
          string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (order == null)
                throw new ArgumentNullException("order");
            ...
            var messageTemplate = GetActiveMessageTemplate("OrderInvoice.CustomerNotification", store.Id);
            if (messageTemplate == null)
                return 0;
        ...
        public string GetZohoOrderBody(Order order, int languageId)
        {
            if (order == null)
                return "";
            ...
            var messageTemplate = GetActiveMessageTemplate("ZohoDesk.OrderPlaced", store.Id);
            if (messageTemplate == null)
                return "";
            ...
            _messageTokenProvider.AddOrderTokens(tokens, order, languageId);
            _messageTokenProvider.AddCustomerTokens(tokens, order.Customer);
            tokens.Add(new Token("Order.CustomerNo", order.Customer.Id, true));
            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);
        ...
        public string GetOrderPlacedBody(Order order, int languageId)
        {
            if (order == null)
                return "";
            ...
            var messageTemplate = GetActiveMessageTemplate("OrderPlaced.AdminHtml", store.Id);
            if (messageTemplate == null)
                return "";
            ...
            var tokens = new List<Token>();
            _messageTokenProvider.AddStoreTokens(tokens, store, emailAccount);
            _messageTokenProvider.AddPurchaseOrderTokens(tokens, order, languageId);
            _messageTokenProvider.AddCustomerTokens(tokens, order.Customer);
            tokens.Add(new Token("Order.CustomerNo", order.Customer.Id, true));
            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);
        ...
        public virtual int ReSendOrderPlacedCustomerNotification(Order order, int languageId,
           string attachmentFilePath = null, string attachmentFileName = null, string resnendToEmail = null)
        {
            if (order == null)
                throw new ArgumentNullException("order");
            ...
            var toName = string.Format("{0} {1}", order.BillingAddress.FirstName, order.BillingAddress.LastName);
            if (!string.IsNullOrEmpty(resnendToEmail))
            {
                toName = resnendToEmail;
                toEmail = resnendToEmail;
            }
            return SendNotification(messageTemplate, emailAccount, languageId, tokens, toEmail, toName,
                attachmentFilePath, attachmentFileName);
        }

        public virtual int ReOrderCustomerNotification(Order order, int languageId, string ReorderComment,
         string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (order == null)
                throw new ArgumentNullException("order");
            ...
            var messageTemplate = GetActiveMessageTemplate("Reorder.CustomerNotification", store.Id);
            if (messageTemplate == null)
                return 0;
            ...
            tokens.Add(new Token("Order.ReorderComment", ReorderComment));
            //event notification
            _eventPublisher.MessageTokensAdded(messageTemplate, tokens);
        ...
        public virtual int ReOrderStoreOwnerNotification(Order order, int languageId, string ReorderComment,
         string attachmentFilePath = null, string attachmentFileName = null)
        {
            if (order == null)
                throw new ArgumentNullException("order");
            ...
            var messageTemplate = GetActiveMessageTemplate("Reorder.StoreOwnerNotification", store.Id);
            if (messageTemplate == null)
                return 0;
