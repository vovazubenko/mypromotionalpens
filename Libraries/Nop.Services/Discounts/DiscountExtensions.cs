...
namespace Nop.Services.Discounts
{
    public static class DiscountExtensions
    {
        ...
        public static List<DiscountForCaching> GetPreferredDiscount(this IList<DiscountForCaching> discounts,
            decimal amount, out decimal discountAmount, int Quantity = 0,bool IsEQP=false)
        {
            if (discounts == null)
            ...
            if (IsEQP == true)
            {
                discounts = discounts.Where(x => x.DiscountAmount == discounts.Max(y => y.DiscountAmount) && x.DiscountTypeId == 2).ToList();
            }
            else {
                discounts = discounts.Where(x => Quantity >= (x.MinimumDiscountedQuantity == null ? 0 : x.MinimumDiscountedQuantity) && Quantity <= ((x.MaximumDiscountedQuantity == null || x.MaximumDiscountedQuantity==0) ? int.MaxValue : x.MaximumDiscountedQuantity)).ToList();
            }
        ...
        public static DiscountForCaching MapDiscount(this Discount discount)
        {
            ...
            return new DiscountForCaching
            {
                ...
                MinimumDiscountAmount = discount.MinimumDiscountAmount,
                ...
                MinimumDiscountedQuantity = discount.MinimumDiscountedQuantity,
